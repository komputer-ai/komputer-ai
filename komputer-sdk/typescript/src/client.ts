...
export class KomputerClient {
  ...
  async createAgent(params: { name: string; instructions: string; allowedTools?: string[]; connectors?: string[]; deleteTTL?: string; disallowedTools?: string[]; lifecycle?: string; sleepTTL?: string; memories?: string[]; model?: string; namespace?: string; officeManager?: string; podSpec?: V1PodSpec; priority?: number; role?: string; secretRefs?: string[]; skills?: string[]; storage?: V1alpha1StorageSpec; systemPrompt?: string; templateRef?: string }) {
    try {
      return await this._agents.createAgent({ request: { allowedTools: params.allowedTools, connectors: params.connectors, deleteTTL: params.deleteTTL, disallowedTools: params.disallowedTools, instructions: params.instructions, lifecycle: params.lifecycle, sleepTTL: params.sleepTTL, memories: params.memories, model: params.model, name: params.name, namespace: params.namespace, officeManager: params.officeManager, podSpec: params.podSpec, priority: params.priority, role: params.role, secretRefs: params.secretRefs, skills: params.skills, storage: params.storage, systemPrompt: params.systemPrompt, templateRef: params.templateRef } });
    } catch (e) {
      if (e instanceof ResponseError && e.response.status === 409) {
        return this._agents.patchAgent({ name: params.name, request: { allowedTools: params.allowedTools, connectors: params.connectors, deleteTTL: params.deleteTTL, disallowedTools: params.disallowedTools, instructions: params.instructions, lifecycle: params.lifecycle, sleepTTL: params.sleepTTL, memories: params.memories, model: params.model, podSpec: params.podSpec, priority: params.priority, secretRefs: params.secretRefs, skills: params.skills, storage: params.storage, systemPrompt: params.systemPrompt, templateRef: params.templateRef } });
      }
      throw e;
    }
  }
  ...
  async patchAgent(params: { name: string; allowedTools?: string[]; connectors?: string[]; deleteTTL?: string; disallowedTools?: string[]; instructions?: string; lifecycle?: string; sleepTTL?: string; memories?: string[]; model?: string; podSpec?: V1PodSpec; priority?: number; secretRefs?: string[]; skills?: string[]; storage?: V1alpha1StorageSpec; systemPrompt?: string; templateRef?: string }) {
    return this._agents.patchAgent({ name: params.name, request: { allowedTools: params.allowedTools, connectors: params.connectors, deleteTTL: params.deleteTTL, disallowedTools: params.disallowedTools, instructions: params.instructions, lifecycle: params.lifecycle, sleepTTL: params.sleepTTL, memories: params.memories, model: params.model, podSpec: params.podSpec, priority: params.priority, secretRefs: params.secretRefs, skills: params.skills, storage: params.storage, systemPrompt: params.systemPrompt, templateRef: params.templateRef } });
  }
