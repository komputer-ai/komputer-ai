...
**task_timeout** | [**V1Duration**](V1Duration.md) | TaskTimeout cancels the agent&#39;s running task once it has been running for this long. The clock starts when a task starts (Status.TaskStartedAt) and never resets — steering a task does not extend it — so this is a hard wall-clock cap on a single task, not an idle timeout.  Only the task is cancelled; the agent itself stays alive and any configured Lifecycle (Sleep / AutoDelete) then applies as it would after any other task end. Unset (default) means tasks run without a time limit. Overrides the template&#39;s taskTimeout when set. +optional | [optional] 
